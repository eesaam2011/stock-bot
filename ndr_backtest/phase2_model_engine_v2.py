...
PROTOCOL_PATH = Path(__file__).with_name("phase2_model_protocol_v2.json")
EMBEDDED_PROTOCOL_JSON = r'''
{
  "protocol_id": "NDR-PHASE2-LOGIT-2026-09-03-B",
  ...
  "model": {
    ...
    "selection_fractions": [0.05, 0.10, 0.20, 0.30],
    "selection_rule": "Select the top frozen fraction by model probability; each internal validation fold is ranked independently",
    ...
    def run_development(self, progress: Callable[[int, int, str | None], None] | None = None) -> dict[str, Any]:
        ...
        for l2 in self.protocol["model"]["l2_penalties"]:
            ...
            for fold_index, (train, valid) in enumerate(folds):
                ...
                oof.extend((row, score, fold_index) for row, score in zip(valid, probability.tolist()))
            ...
            y_oof = np.asarray([row["explosion_ge10"] for row, _, _ in oof], dtype=int)
            p_oof = np.asarray([probability for _, probability, _ in oof], dtype=float)
            for fraction in self.protocol["model"]["selection_fractions"]:
                selected = []
                fold_cutoffs = []
                for fold_index in range(len(folds)):
                    fold_rows = [(row, probability) for row, probability, index in oof if index == fold_index]
                    cutoff = float(np.quantile([probability for _, probability in fold_rows], 1.0 - float(fraction)))
                    fold_selected = [row for row, probability in fold_rows if probability >= cutoff]
                    selected.extend(fold_selected)
                    fold_cutoffs.append({
                        "fold": fold_index + 1,
                        "probability_cutoff": round(cutoff, 10),
                        "selected_rows": len(fold_selected),
                    })
                performance = self._performance(selected)
                candidates.append({
                    "l2_penalty": float(l2), "selection_fraction": float(fraction),
                    "oof_rows": len(oof),
                    "oof_classification_auc": round(self._auc(y_oof, p_oof), 6) if self._auc(y_oof, p_oof) is not None else None,
                    "fold_probability_cutoffs": fold_cutoffs,
        ...
        if selected_spec is not None:
            ...
            development_probability = self._predict(model, self._matrix(development))
            deployment_cutoff = float(np.quantile(
                development_probability, 1.0 - selected_spec["selection_fraction"]
            ))
            fitted_model["frozen_probability_cutoff"] = round(deployment_cutoff, 10)
            if legacy_holdout:
                probability = self._predict(model, self._matrix(legacy_holdout))
                selected = [row for row, score in zip(legacy_holdout, probability) if score >= deployment_cutoff]
        ...
        report = {
            ...
            "dataset": {
                ...
                "development_explosion_ge10_count": sum(row["explosion_ge10"] for row in development),
                "development_explosion_ge10_rate_pct": round(sum(row["explosion_ge10"] for row in development) / max(1, len(development)) * 100, 6),
                "legacy_holdout_explosion_ge10_count": sum(row["explosion_ge10"] for row in legacy_holdout),
                "legacy_holdout_explosion_ge10_rate_pct": round(sum(row["explosion_ge10"] for row in legacy_holdout) / max(1, len(legacy_holdout)) * 100, 6),
            },
