...
EARLY_FEATURE_RANKING_CONSTRUCTION_SPEC = {
    "execution_id":"IPR-EARLY-FEATURE-EQUAL-WEIGHT-RANKING-CONSTRUCTION-2026-09-14-A",
    "required_ranking_prefreeze_sha256":"47c9a2100101ca793c8b2eeb9ecbe156d5b63c553cf517360264d4166afda299",
    "required_reconstruction_result_sha256":"d1a4af68ebb3af814b0358b819513912b89d53cf973f06233c5bbccdfc6b1e1c",
    "required_feature_definition_sha256":"f615c7181c78da91e89c6ac7fa8608958043a4e029c52c76954d39ceaaee6b71",
    "reference_period":{"start":"2019-01-01","end":"2024-12-31"},
    "features":["discovery_range_pct","log_discovery_volume","return_5m_pct"],
    "windows_minutes":[30,60],
    "weights":{"discovery_range_pct":1.0/3.0,"log_discovery_volume":1.0/3.0,"return_5m_pct":1.0/3.0},
    "standardization":"Equal-symbol weighted mean and population SD over all causally available 2019-2024 reconstruction observations, separately per Feature x Window. Each symbol has total weight 1, split equally across its observations.",
    "ranking_formula":"rank_score=(z(discovery_range_pct)+z(log_discovery_volume)+z(return_5m_pct))/3",
    "scoreability":"All three frozen features must be available. No imputation or partial-score renormalization.",
    "outputs":"Frozen standardization constants and per-record score fields only. No label-conditioned ranking performance is computed or reported.",
    "decision_rule":"NONE. Construction only; no threshold, top-k, PASS/FAIL, profitability, or downstream authorization.",
...
EARLY_FEATURE_RANKING_EVALUATION_EXEC_SPEC = {
    "execution_id":"IPR-EARLY-FEATURE-RANKING-EVALUATION-EXECUTION-2026-09-14-A",
    "required_prefreeze_sha256":"e99c7d5d8f169f8b038262d6bf1daaccdc6d51bbbad10260d04639813b75b555",
    "required_ranking_construction_result_sha256":"00cb344337ca25c7e644adebf3aae73b8bd8c81bc986e10240ac2650295ce300",
    "evaluation_period":{"start":"2019-01-01","end":"2024-12-31"},
    "windows_minutes":[30,60],
    "primary_metric":"equal_symbol_weighted_roc_auc",
    "minimum_pooled_auc":0.55,
    "minimum_years_auc_above_0_50":5,
    "support_minima":{"positive_events":500,"hard_negative_events":500,"positive_symbols":100,"hard_negative_symbols":100},
    "decision_rule":"DISCOVERY_RANKING_GO iff BOTH windows satisfy pooled AUC >=0.55, frozen support minima, and annual AUC >0.50 in at least 5 of 6 years. Otherwise DISCOVERY_RANKING_NO_GO. No rounding.",
...
EARLY_FEATURE_TWO_COMPONENT_2025_VALIDATION_EXEC_SPEC = {
    ...
    "required_hypothesis_prefreeze_sha256":"59154c2b573a991a31928cc33ee8dfe1d45ab7e26a716b8eda87041e1db0ee74",
    "required_post_failure_diagnostic_result_sha256":"2c55b7f1714c5c1750c4f032ae457c8cbdf2343182d1954cc6bdf2f9144fe866",
    "required_2025_feature_validation_result_sha256":"8d314542cedff0a0897a6e45c10bbf3b2524d6efd1f8322434c360a66b4af2f3",
    "required_ranking_construction_result_sha256":"00cb344337ca25c7e644adebf3aae73b8bd8c81bc986e10240ac2650295ce300",
    ...
    "weights":{"discovery_range_pct":0.5,"return_5m_pct":0.5},
    "windows_minutes":[30,60],
    "data_source":"Reuse the already-persisted causal 2025 feature-validation records from v1.7.34; no new Alpaca request. Reuse v1.7.37-R1 2019-2024 Discovery-only standardization constants without refit.",
    "primary_metric":"equal_symbol_weighted_roc_auc",
    "gate":{"minimum_auc_each_window":0.55,"minimum_auc_strictly_above_each_window":0.50,"support_minima_each_window":{"positive_events":500,"hard_negative_events":500,"positive_symbols":100,"hard_negative_symbols":100},"overall_rule":"VALIDATION_PASS iff BOTH 30m and 60m have AUC >= 0.55, AUC > 0.50, and all support minima. No rounding. Otherwise VALIDATION_FAIL and STOP REVIEW."},
    "missingness":"Score only when both frozen raw components are available and finite. No imputation, no partial renormalization.",
    "forbidden":["2019-2024 retest","alternative subset search","feature change","weight change","threshold search","top-k optimization","best-window selection","dropping 30m or 60m","2026 ranking performance read","Fresh OOS read","trading optimization","profitability claims","automatic downstream authorization"],
    "guardrails":{"alpaca_requests":False,"ranking_threshold_defined":False,"top_k_optimized":False,"ranking_review_2026_opened":False,"fresh_oos_opened":False,"strategy_pass":False,"bot_authorized":False,"automatic_downstream_authorization":False,"stop_and_review_required":True}
}
EARLY_FEATURE_TWO_COMPONENT_2025_VALIDATION_EXEC_SPEC_SHA256=hashlib.sha256(json.dumps(EARLY_FEATURE_TWO_COMPONENT_2025_VALIDATION_EXEC_SPEC,sort_keys=True,separators=(",",":"),allow_nan=False).encode()).hexdigest()
...
class IndependentPriorityRadar:
    def __init__(self, redis_client: RedisREST | None = None, alpaca: AlpacaClient | None = None):
        ...
        self.early_feature_ranking_construction_lock=threading.RLock(); self.early_feature_ranking_construction_thread=None
    ...
    def early_feature_ranking_evaluation_key(self,suffix: str)->str:
        return self.key(f"early_feature_ranking_evaluation:v1:{suffix}")

    def _set_early_feature_ranking_evaluation_state(self,**updates: Any)->None:
        with self.early_feature_ranking_evaluation_lock:
            self.early_feature_ranking_evaluation_state.update(updates); self.early_feature_ranking_evaluation_state["updated_at"]=iso(); snap=dict(self.early_feature_ranking_evaluation_state)
        if self.redis.configured:self.redis.set_json(self.early_feature_ranking_evaluation_key("status"),snap)

    def _early_feature_ranking_evaluation_gate(self):
        if not self.redis.configured:return False,"Redis required"
        if EARLY_FEATURE_RANKING_EVALUATION_PREFREEZE_SHA256!=EARLY_FEATURE_RANKING_EVALUATION_EXEC_SPEC["required_prefreeze_sha256"]:return False,"Ranking evaluation pre-freeze SHA mismatch"
        rr=self.redis.get_json(self.early_feature_ranking_construction_key("report"),{}) or {}
        if rr.get("status")!="COMPLETED" or rr.get("result_sha256")!=EARLY_FEATURE_RANKING_EVALUATION_EXEC_SPEC["required_ranking_construction_result_sha256"]:return False,"Ranking construction provenance mismatch"
        if rr.get("ranking_performance_computed") is not False or rr.get("ranking_threshold_defined") is not False:return False,"Ranking construction already opened forbidden performance/threshold"
        if rr.get("fresh_oos_opened") or rr.get("post_2026_08_31_read"):return False,"Ranking construction provenance opened forbidden data"
        return True,"allowed"

    @staticmethod
    def _equal_symbol_weighted_auc(rows):
        # rows: (score, class, symbol). Reweight independently within each class so every symbol totals weight 1.
        counts={"positive":{},"hard_negative":{}}
        for score,cls,sym in rows:
            if cls in counts:counts[cls][sym]=counts[cls].get(sym,0)+1
        if not counts["positive"] or not counts["hard_negative"]:return None
        weighted=[]
        for score,cls,sym in rows:
            if cls in counts and sym in counts[cls]:weighted.append((float(score),cls,1.0/counts[cls][sym]))
        total_p=sum(w for _,c,w in weighted if c=="positive"); total_n=sum(w for _,c,w in weighted if c=="hard_negative")
        if total_p<=0 or total_n<=0:return None
        weighted.sort(key=lambda x:x[0]); cum_n=0.0; numer=0.0; i=0
        while i<len(weighted):
            score=weighted[i][0]; j=i; tie_p=tie_n=0.0
            while j<len(weighted) and weighted[j][0]==score:
                if weighted[j][1]=="positive":tie_p+=weighted[j][2]
                else:tie_n+=weighted[j][2]
                j+=1
            numer+=tie_p*(cum_n+0.5*tie_n); cum_n+=tie_n; i=j
        return numer/(total_p*total_n)

    @staticmethod
    def _weighted_class_summary(rows,cls):
        sub=[(float(sc),sym) for sc,c,sym in rows if c==cls]
        if not sub:return {"events":0,"symbols":0,"mean":None,"median":None}
        cnt={}
        for _,sym in sub:cnt[sym]=cnt.get(sym,0)+1
        vals=sorted((sc,1.0/cnt[sym]) for sc,sym in sub); tw=sum(w for _,w in vals)
        mean=sum(sc*w for sc,w in vals)/tw; acc=0.0; med=vals[-1][0]
        for sc,w in vals:
            acc+=w
            if acc>=0.5*tw:med=sc;break
        return {"events":len(sub),"symbols":len(cnt),"mean":mean,"median":med}

    def early_feature_ranking_evaluation_loop(self):
        try:
            ok,why=self._early_feature_ranking_evaluation_gate()
            if not ok:raise RuntimeError(why)
            sessions=sorted(self.redis.get_json(self.early_feature_reconstruction_key("completed_sessions"),[]) or [])
            sessions=[str(x) for x in sessions if "2019-01-01"<=str(x)<="2024-12-31"]
            wins=list(EARLY_FEATURE_RANKING_EVALUATION_EXEC_SPEC["windows_minutes"]); years=[2019,2020,2021,2022,2023,2024]
            self._set_early_feature_ranking_evaluation_state(status="RUNNING",phase="RANKING_DISCOVERY_EVALUATION",message=f"Ranking evaluation 0/{len(sessions)} sessions",sessions_completed=0,total_sessions=len(sessions),alpaca_requests_made=0,fresh_oos_opened=False)
            pooled={w:[] for w in wins}; annual={(w,y):[] for w in wins for y in years}; eligible={(w,c):0 for w in wins for c in ["positive","hard_negative"]}; unavailable={(w,c):0 for w in wins for c in ["positive","hard_negative"]}
            for idx,sess in enumerate(sessions,1):
                ...
                seen=set()
                for q in scores:
                    rid=q.get("record_id"); lab=labels.get(rid); score=q.get("rank_score")
                    if not lab:continue
                    c,sym,w=lab
                    if w not in wins or c not in ("positive","hard_negative"):continue
                    seen.add(rid)
                    if not isinstance(score,(int,float)) or not math.isfinite(float(score)):continue
                    row=(float(score),c,sym);pooled[w].append(row);annual[(w,int(sess[:4]))].append(row)
                # Unavailability is eligible minus scoreable, accumulated after all sessions via pooled counts.
                if idx%100==0:self._set_early_feature_ranking_evaluation_state(status="RUNNING",phase="RANKING_DISCOVERY_EVALUATION",message=f"Ranking evaluation {idx}/{len(sessions)} sessions",sessions_completed=idx,total_sessions=len(sessions),alpaca_requests_made=0,fresh_oos_opened=False)
            results=[]; all_pass=True; support_min=EARLY_FEATURE_RANKING_EVALUATION_EXEC_SPEC["support_minima"]
            for w in wins:
                rows=pooled[w]; auc=self._equal_symbol_weighted_auc(rows); ps=self._weighted_class_summary(rows,"positive"); hs=self._weighted_class_summary(rows,"hard_negative")
                yearly=[]; positive_years=0
                for y in years:
                    a=self._equal_symbol_weighted_auc(annual[(w,y)]); yearly.append({"year":y,"auc":a}); positive_years+=int(a is not None and a>0.50)
                support_ok=(ps["events"]>=support_min["positive_events"] and hs["events"]>=support_min["hard_negative_events"] and ps["symbols"]>=support_min["positive_symbols"] and hs["symbols"]>=support_min["hard_negative_symbols"])
                pooled_ok=auc is not None and auc>=EARLY_FEATURE_RANKING_EVALUATION_EXEC_SPEC["minimum_pooled_auc"]
                stability_ok=positive_years>=EARLY_FEATURE_RANKING_EVALUATION_EXEC_SPEC["minimum_years_auc_above_0_50"]
                passed=bool(pooled_ok and support_ok and stability_ok);all_pass=all_pass and passed
                results.append({"window_minutes":w,"pooled_auc":auc,"passes_pooled_auc":pooled_ok,"positive_years_auc_above_0_50":positive_years,"passes_annual_stability":stability_ok,"passes_support":support_ok,"passes_discovery_ranking_gate":passed,"support":{"positive_events":ps["events"],"positive_symbols":ps["symbols"],"hard_negative_events":hs["events"],"hard_negative_symbols":hs["symbols"]},"class_score_summary":{"positive":{"mean":ps["mean"],"median":ps["median"]},"hard_negative":{"mean":hs["mean"],"median":hs["median"]}},"annual_auc":yearly,"coverage":{"positive":{"eligible":eligible[(w,"positive")],"scoreable":ps["events"],"rate":ps["events"]/eligible[(w,"positive")] if eligible[(w,"positive")] else None},"hard_negative":{"eligible":eligible[(w,"hard_negative")],"scoreable":hs["events"],"rate":hs["events"]/eligible[(w,"hard_negative")] if eligible[(w,"hard_negative")] else None}}})
            decision="DISCOVERY_RANKING_GO" if all_pass else "DISCOVERY_RANKING_NO_GO"
            report={"version":VERSION,"build":BUILD,"execution_id":EARLY_FEATURE_RANKING_EVALUATION_EXEC_SPEC["execution_id"],"execution_spec_sha256":EARLY_FEATURE_RANKING_EVALUATION_EXEC_SPEC_SHA256,"required_prefreeze_sha256":EARLY_FEATURE_RANKING_EVALUATION_EXEC_SPEC["required_prefreeze_sha256"],"required_ranking_construction_result_sha256":EARLY_FEATURE_RANKING_EVALUATION_EXEC_SPEC["required_ranking_construction_result_sha256"],"status":"COMPLETED","phase":"RANKING_EVALUATION_STOP_REVIEW","decision":decision,"results":results,"sessions":len(sessions),"alpaca_requests_made":0,"ranking_threshold_defined":False,"top_k_optimized":False,"ranking_validation_2025_opened":False,"ranking_review_2026_opened":False,"fresh_oos_opened":False,"strategy_pass":False,"bot_authorized":False,"automatic_downstream_authorization":False,"stop_and_review_required":True,"completed_at":iso()}
            canon=dict(report);canon.pop("completed_at");report["result_sha256"]=hashlib.sha256(json.dumps(canon,sort_keys=True,separators=(",",":"),allow_nan=False).encode()).hexdigest();self.redis.set_json(self.early_feature_ranking_evaluation_key("report"),report)
            self._set_early_feature_ranking_evaluation_state(status="COMPLETED",phase="RANKING_EVALUATION_STOP_REVIEW",message=f"{decision}; STOP REVIEW",decision=decision,sessions_completed=len(sessions),total_sessions=len(sessions),alpaca_requests_made=0,ranking_validation_2025_opened=False,ranking_review_2026_opened=False,fresh_oos_opened=False,stop_and_review_required=True)
        except Exception as e:
            logging.exception("Ranking evaluation failed");self._set_early_feature_ranking_evaluation_state(status="ERROR",phase="RANKING_EVALUATION_BLOCKED",message=f"{type(e).__name__}: {e}",alpaca_requests_made=0,fresh_oos_opened=False)
        finally:
            with self.early_feature_ranking_evaluation_lock:self.early_feature_ranking_evaluation_thread=None

    def start_early_feature_ranking_evaluation(self):
        ok,why=self._early_feature_ranking_evaluation_gate()
        if not ok:return False,why
        with self.early_feature_ranking_evaluation_lock:
            if self.early_feature_ranking_evaluation_thread and self.early_feature_ranking_evaluation_thread.is_alive():return False,"already_running"
            existing=self.redis.get_json(self.early_feature_ranking_evaluation_key("report"),None) if self.redis.configured else None
            if existing and existing.get("status")=="COMPLETED":return False,"already_completed"
            self.early_feature_ranking_evaluation_thread=threading.Thread(target=self.early_feature_ranking_evaluation_loop,name="ranking-evaluation",daemon=True);self.early_feature_ranking_evaluation_thread.start()
        return True,"started"


    def early_feature_two_component_2025_validation_key(self,suffix: str)->str:
        return self.key(f"early_feature_two_component_2025_validation:v1:{suffix}")

    def _set_early_feature_two_component_2025_validation_state(self,**updates: Any)->None:
        with self.early_feature_two_component_2025_validation_lock:
            self.early_feature_two_component_2025_validation_state.update(updates); self.early_feature_two_component_2025_validation_state["updated_at"]=iso(); snap=dict(self.early_feature_two_component_2025_validation_state)
        if self.redis.configured:self.redis.set_json(self.early_feature_two_component_2025_validation_key("status"),snap)

    def _early_feature_two_component_2025_validation_gate(self):
        if not self.redis.configured:return False,"Redis required"
        spec=EARLY_FEATURE_TWO_COMPONENT_2025_VALIDATION_EXEC_SPEC
        if EARLY_FEATURE_TWO_COMPONENT_RANKING_HYPOTHESIS_PREFREEZE_SHA256!=spec["required_hypothesis_prefreeze_sha256"]:return False,"v1.7.42 hypothesis pre-freeze SHA mismatch"
        diag=self.redis.get_json(self.early_feature_post_failure_ranking_diagnostic_key("report"),{}) or {}
        if diag.get("status")!="COMPLETED" or diag.get("result_sha256")!=spec["required_post_failure_diagnostic_result_sha256"]:return False,"v1.7.41 diagnostic provenance mismatch"
        if diag.get("v1_7_39_decision_immutable")!="DISCOVERY_RANKING_NO_GO":return False,"v1.7.39 immutable decision mismatch"
        vr=self.redis.get_json(self.early_feature_2025_validation_key("report"),{}) or {}
        if vr.get("status")!="COMPLETED" or vr.get("result_sha256")!=spec["required_2025_feature_validation_result_sha256"]:return False,"Persisted causal 2025 feature records provenance mismatch"
        if vr.get("fresh_oos_opened") or vr.get("year_2026_opened"):return False,"2025 feature source opened forbidden later data"
        rr=self.redis.get_json(self.early_feature_ranking_construction_key("report"),{}) or {}
        if rr.get("status")!="COMPLETED" or rr.get("result_sha256")!=spec["required_ranking_construction_result_sha256"]:return False,"Frozen ranking constants provenance mismatch"
        if rr.get("ranking_performance_computed") is not False or rr.get("labels_used_for_reference_fit") is not False:return False,"Ranking reference provenance contaminated"
        return True,"allowed"

    def early_feature_two_component_2025_validation_loop(self):
        try:
            ok,why=self._early_feature_two_component_2025_validation_gate()
            if not ok:raise RuntimeError(why)
            spec=EARLY_FEATURE_TWO_COMPONENT_2025_VALIDATION_EXEC_SPEC; wins=set(spec["windows_minutes"]); feats=list(spec["components"])
            sessions=sorted(str(x) for x in (self.redis.get_json(self.early_feature_2025_validation_key("completed_sessions"),[]) or []) if str(x).startswith("2025-"))
            ...
            self._set_early_feature_two_component_2025_validation_state(status="RUNNING",phase="TWO_COMPONENT_2025_LOCKED_VALIDATION",message=f"2025 locked ranking validation 0/{len(sessions)} sessions",sessions_completed=0,total_sessions=len(sessions),alpaca_requests_made=0,ranking_validation_2025_opened=True,ranking_review_2026_opened=False,fresh_oos_opened=False)
            ...
            for idx,sess in enumerate(sessions,1):
                ...
                if idx%25==0 or idx==len(sessions):self._set_early_feature_two_component_2025_validation_state(status="RUNNING",phase="TWO_COMPONENT_2025_LOCKED_VALIDATION",message=f"2025 locked ranking validation {idx}/{len(sessions)} sessions",sessions_completed=idx,total_sessions=len(sessions),alpaca_requests_made=0,ranking_validation_2025_opened=True,ranking_review_2026_opened=False,fresh_oos_opened=False)
            ...
            for w in sorted(wins):
                ...
                results.append({"window_minutes":w,"equal_symbol_weighted_roc_auc":auc,"minimum_auc_required":gate["minimum_auc_each_window"],"support":support,"support_pass":support_pass,"coverage":coverage,"passes_2025_gate":passed})
            decision="VALIDATION_PASS" if len(results)==2 and all(x["passes_2025_gate"] for x in results) else "VALIDATION_FAIL"
            report={"version":VERSION,"build":BUILD,"execution_id":spec["execution_id"],"execution_spec_sha256":EARLY_FEATURE_TWO_COMPONENT_2025_VALIDATION_EXEC_SPEC_SHA256,"required_hypothesis_prefreeze_sha256":spec["required_hypothesis_prefreeze_sha256"],"required_post_failure_diagnostic_result_sha256":spec["required_post_failure_diagnostic_result_sha256"],"required_2025_feature_validation_result_sha256":spec["required_2025_feature_validation_result_sha256"],"required_ranking_construction_result_sha256":spec["required_ranking_construction_result_sha256"],"status":"COMPLETED","phase":"TWO_COMPONENT_2025_LOCKED_VALIDATION_STOP_REVIEW","candidate_name":spec["candidate_name"],"formula":spec["formula"],"validation_year":2025,"sessions":len(sessions),"results":results,"decision":decision,"alpaca_requests_made":0,"ranking_validation_2025_opened":True,"ranking_review_2026_opened":False,"fresh_oos_opened":False,"ranking_threshold_defined":False,"top_k_optimized":False,"strategy_pass":False,"bot_authorized":False,"automatic_downstream_authorization":False,"stop_and_review_required":True,"completed_at":iso()}
            canon=dict(report);canon.pop("completed_at");report["result_sha256"]=hashlib.sha256(json.dumps(canon,sort_keys=True,separators=(",",":"),allow_nan=False).encode()).hexdigest();self.redis.set_json(self.early_feature_two_component_2025_validation_key("report"),report)
            self._set_early_feature_two_component_2025_validation_state(status="COMPLETED",phase="TWO_COMPONENT_2025_LOCKED_VALIDATION_STOP_REVIEW",message=f"2025 {decision}; STOP REVIEW",decision=decision,sessions_completed=len(sessions),total_sessions=len(sessions),alpaca_requests_made=0,ranking_validation_2025_opened=True,ranking_review_2026_opened=False,fresh_oos_opened=False,stop_and_review_required=True)
        except Exception as e:
            logging.exception("Two-component 2025 locked validation failed");self._set_early_feature_two_component_2025_validation_state(status="ERROR",phase="TWO_COMPONENT_2025_LOCKED_VALIDATION_BLOCKED",message=f"{type(e).__name__}: {e}",alpaca_requests_made=0,ranking_validation_2025_opened=True,ranking_review_2026_opened=False,fresh_oos_opened=False)
        finally:
            with self.early_feature_two_component_2025_validation_lock:self.early_feature_two_component_2025_validation_thread=None

    def start_early_feature_two_component_2025_validation(self):
        ok,why=self._early_feature_two_component_2025_validation_gate()
        if not ok:return False,why
        existing=self.redis.get_json(self.early_feature_two_component_2025_validation_key("report"),None) if self.redis.configured else None
        if existing and existing.get("status")=="COMPLETED":return False,"already_completed"
        with self.early_feature_two_component_2025_validation_lock:
            if self.early_feature_two_component_2025_validation_thread and self.early_feature_two_component_2025_validation_thread.is_alive():return False,"already_running"
...
@app.get("/research/early-causal-entry/feature-level-discovery/two-component-ranking-2025-validation/protocol")
def early_feature_two_component_2025_validation_protocol():
...
@app.get("/research/early-causal-entry/feature-level-discovery/two-component-ranking-2025-validation/status")
def early_feature_two_component_2025_validation_status():
    x=radar.redis.get_json(radar.early_feature_two_component_2025_validation_key("status"),None) if radar.redis.configured else None
    with radar.early_feature_two_component_2025_validation_lock:out=dict(x or radar.early_feature_two_component_2025_validation_state);out["worker_alive"]=bool(radar.early_feature_two_component_2025_validation_thread and radar.early_feature_two_component_2025_validation_thread.is_alive())
    return jsonify(out)

@app.get("/research/early-causal-entry/feature-level-discovery/two-component-ranking-2025-validation/result")
def early_feature_two_component_2025_validation_result():
    x=radar.redis.get_json(radar.early_feature_two_component_2025_validation_key("report"),None) if radar.redis.configured else None
    if not x:return jsonify({"result_ready":False,"status_url":"/research/early-causal-entry/feature-level-discovery/two-component-ranking-2025-validation/status"}),202
    return jsonify(x)
...
@app.get("/research/early-causal-entry/feature-level-discovery/ranking-evaluation/protocol")
def research_feature_ranking_evaluation_protocol():
    allowed,reason=radar._early_feature_ranking_evaluation_gate(); rr=radar.redis.get_json(radar.early_feature_ranking_construction_key("report"),{}) if radar.redis.configured else {}
    return jsonify({"version":VERSION,"build":BUILD,"execution_spec":EARLY_FEATURE_RANKING_EVALUATION_EXEC_SPEC,"execution_spec_sha256":EARLY_FEATURE_RANKING_EVALUATION_EXEC_SPEC_SHA256,"required_prefreeze_sha256":EARLY_FEATURE_RANKING_EVALUATION_EXEC_SPEC["required_prefreeze_sha256"],"required_ranking_construction_result_sha256":EARLY_FEATURE_RANKING_EVALUATION_EXEC_SPEC["required_ranking_construction_result_sha256"],"actual_ranking_construction_result_sha256":rr.get("result_sha256"),"gate_allowed":allowed,"gate_reason":reason,"alpaca_authorized":False,"alpaca_requests_made":0,"ranking_performance_read_by_protocol":False,"ranking_threshold_defined":False,"ranking_validation_2025_opened":False,"ranking_review_2026_opened":False,"fresh_oos_opened":False})

@app.get("/research/early-causal-entry/feature-level-discovery/ranking-evaluation/start")
def research_feature_ranking_evaluation_start():
    ok,why=radar.start_early_feature_ranking_evaluation();return jsonify({"ok":ok,"status":"started" if ok else why,"status_url":"/research/early-causal-entry/feature-level-discovery/ranking-evaluation/status","result_url":"/research/early-causal-entry/feature-level-discovery/ranking-evaluation/result"}),(202 if ok else 409)

@app.get("/research/early-causal-entry/feature-level-discovery/ranking-evaluation/status")
def research_feature_ranking_evaluation_status():
    x=radar.redis.get_json(radar.early_feature_ranking_evaluation_key("status"),None) if radar.redis.configured else None
    with radar.early_feature_ranking_evaluation_lock:out=dict(x or radar.early_feature_ranking_evaluation_state);out["worker_alive"]=bool(radar.early_feature_ranking_evaluation_thread and radar.early_feature_ranking_evaluation_thread.is_alive())
    return jsonify(out)

@app.get("/research/early-causal-entry/feature-level-discovery/ranking-evaluation/result")
def research_feature_ranking_evaluation_result():
    x=radar.redis.get_json(radar.early_feature_ranking_evaluation_key("report"),None) if radar.redis.configured else None
    return jsonify(x) if x else (jsonify({"result_ready":False,"status_url":"/research/early-causal-entry/feature-level-discovery/ranking-evaluation/status"}),202)
...
def research_feature_ranking_construction_protocol():
    allowed,reason=radar._early_feature_ranking_construction_gate();rr=radar.redis.get_json(radar.early_feature_reconstruction_key("report"),{}) if radar.redis.configured else {}
    return jsonify({"version":VERSION,"build":BUILD,"execution_spec":EARLY_FEATURE_RANKING_CONSTRUCTION_SPEC,"execution_spec_sha256":EARLY_FEATURE_RANKING_CONSTRUCTION_SPEC_SHA256,"required_ranking_prefreeze_sha256":EARLY_FEATURE_RANKING_CONSTRUCTION_SPEC["required_ranking_prefreeze_sha256"],"actual_ranking_prefreeze_sha256":EARLY_FEATURE_RANKING_PREFREEZE_SHA256,"required_reconstruction_result_sha256":EARLY_FEATURE_RANKING_CONSTRUCTION_SPEC["required_reconstruction_result_sha256"],"actual_reconstruction_result_sha256":rr.get("result_sha256"),"gate_allowed":allowed,"gate_reason":reason,"alpaca_authorized":False,"alpaca_requests_made":0,"ranking_performance_read_by_protocol":False,"ranking_threshold_defined":False,"validation_2025_opened":False,"year_2026_opened":False,"fresh_oos_opened":False,"post_2026_08_31_read":False})

@app.route("/research/early-causal-entry/feature-level-discovery/ranking-construction/start",methods=["GET","POST"])
def research_feature_ranking_construction_start():
    ok,why=radar.start_early_feature_ranking_construction();return jsonify({"ok":ok,"status":"started" if ok else why,"status_url":"/research/early-causal-entry/feature-level-discovery/ranking-construction/status","result_url":"/research/early-causal-entry/feature-level-discovery/ranking-construction/result"}),(202 if ok else 409)

@app.get("/research/early-causal-entry/feature-level-discovery/ranking-construction/status")
def research_feature_ranking_construction_status():
    x=radar.redis.get_json(radar.early_feature_ranking_construction_key("status"),None) if radar.redis.configured else None
    with radar.early_feature_ranking_construction_lock:out=dict(x or radar.early_feature_ranking_construction_state);out["worker_alive"]=bool(radar.early_feature_ranking_construction_thread and radar.early_feature_ranking_construction_thread.is_alive())
    return jsonify(out)

@app.get("/research/early-causal-entry/feature-level-discovery/ranking-construction/result")
def research_feature_ranking_construction_result():
    x=radar.redis.get_json(radar.early_feature_ranking_construction_key("report"),None) if radar.redis.configured else None
    if not x:return jsonify({"result_ready":False,"status_url":"/research/early-causal-entry/feature-level-discovery/ranking-construction/status"}),202
    return jsonify(x)
...
# ---------------------------------------------------------------------------
# v1.7.54 — 2018 Observed Historical SIP Universe certification
# Methodology-alignment audit only. This does NOT open 2018 H1 outcomes.
# ---------------------------------------------------------------------------

BACKWARD_OOS_2018_OBSERVED_CERT_SPEC = {
    "certification_id": "IPR-2018-OBSERVED-HISTORICAL-SIP-UNIVERSE-CERTIFICATION-2026-09-14-A",
    "required_2018_reconstruction_result_sha256": "4eb227f00e6518fa7810db9fe4391c13d5701971c87710c48b0a46f5d1ee81ab",
    "required_2018_reconstruction_spec_sha256": "c9276080a32128f941195876745ec767ab7c72925428285f4b9825fd997eae7b",
    "required_2019_2026_reconstruction_sha256": "c2a8bd08d7023b093de7a98fd6713131b909f0512f3fa769ac84f203f2e6a6f5",
    "terminology": {
        "2018": "Observed Historical SIP Universe",
        "2019_2026": "Observed Historical SIP Universe",
        "explicit_limitation": "Neither period is claimed to be a complete/certified US-market point-in-time Security Master universe.",
        "retroactive_note_only": True,
        "historical_results_mutated": False,
    },
    "2018_expected": {"source_union_symbols": 31959, "symbols_observed_in_sip": 6960},
    "2019_2026_frozen_reference": {
        "union_symbols": 31909,
        "symbols_with_sip_presence": 14677,
        "year_presence_counts": {"2019":7251,"2020":7651,"2021":8381,"2022":8465,"2023":8756,"2024":9652,"2025":11330,"2026":12742},
    },
    "equivalence_checks": [
        "source-union reconstruction with retained provenance",
        "Alpaca SIP raw 1Month presence establishes observed symbol-year presence",
        "ticker text is not permanent entity identity",
        "no cross-era entity merge",
        "candidate-stage split suspect screen must reuse the original adjacent coarse close >=3.5x logic unchanged",
        "split suspects must remain ineligible for downstream candidate verification",
    ],
    "split_warmup_finding": {
        "finding": "NO_LONG_HISTORY_DEPENDENCY",
        "reason": "The frozen _phase0a_split_suspect screen uses only adjacent closes inside the fetched trading cycle; it does not estimate a long-history baseline.",
        "dec2017_warmup_required_for_this_screen": False,
        "screen_is_complete_corporate_action_certificate": False,
        "phase0b_strict_exclusion_still_required": True,
    },
    "diagnostic_policy": {
        "compare_2018_to_2019": True,
        "compare_2018_to_2019_2026_annual_mean": True,
        "diagnostics_are_coverage_proof": False,
        "diagnostics_are_pass_fail_gates": False,
        "warning": "Annual presence counts mix market evolution, listings/delistings, source observability and possible edge effects; they do not identify true-market completeness.",
    },
    "certification_gate": {
        "requires_exact_2018_reconstruction_provenance": True,
        "requires_exact_2019_2026_reconstruction_provenance": True,
        "requires_recomputed_2018_record_counts_match": True,
        "requires_methodology_alignment_checks": True,
        "independent_security_master_required": False,
        "coverage_pct_threshold": None,
    },
    "firewall": {
        "alpaca_requests": False,
        "h1_computed": False,
        ...
        "backward_oos_opened": False,
        "fresh_forward_oos_opened": False,
        "2019_2026_results_mutated": False,
    },
    "decision_policy": (
        "OBSERVED_UNIVERSE_CERTIFIED iff the exact completed v1.7.53 reconstruction and exact frozen v1.7.1 2019-2026 reconstruction are present, "
        "the persisted 2018 records recompute to the frozen counts, and methodology-alignment checks pass. "
        "This certifies methodological comparability of the observed universe only; it does not certify complete-market coverage, corporate actions, H1, or trading validity."
    ),
}
BACKWARD_OOS_2018_OBSERVED_CERT_SPEC_SHA256 = hashlib.sha256(
    json.dumps(BACKWARD_OOS_2018_OBSERVED_CERT_SPEC, sort_keys=True, separators=(",", ":"), allow_nan=False).encode()
).hexdigest()
BACKWARD_OOS_2018_OBSERVED_CERT_LOCK = threading.RLock()
BACKWARD_OOS_2018_OBSERVED_CERT_THREAD = None
BACKWARD_OOS_2018_OBSERVED_CERT_STATE = {
    "status":"IDLE", "phase":"NOT_STARTED", "message":"2018 observed-universe certification has not started",
    "certification_id":BACKWARD_OOS_2018_OBSERVED_CERT_SPEC["certification_id"], "updated_at":iso(),
    "h1_computed":False, "backward_oos_opened":False, "fresh_forward_oos_opened":False,
}

def _boos18oc_key(suffix: str) -> str:
    return radar.key(f"backward_oos_2018_observed_cert:v1:{suffix}")

def _boos18oc_set_state(**updates: Any) -> None:
    global BACKWARD_OOS_2018_OBSERVED_CERT_STATE
    with BACKWARD_OOS_2018_OBSERVED_CERT_LOCK:
        BACKWARD_OOS_2018_OBSERVED_CERT_STATE = {**BACKWARD_OOS_2018_OBSERVED_CERT_STATE, **updates, "updated_at":iso(),
            "h1_computed":False,"backward_oos_opened":False,"fresh_forward_oos_opened":False}
        snap=dict(BACKWARD_OOS_2018_OBSERVED_CERT_STATE)
    if radar.redis.configured: radar.redis.set_json(_boos18oc_key("status"), snap)

def _boos18oc_worker() -> None:
    global BACKWARD_OOS_2018_OBSERVED_CERT_THREAD
    try:
        if not radar.redis.configured: raise RuntimeError("Redis is required")
        _boos18oc_set_state(status="RUNNING",phase="PROVENANCE",message="Verifying frozen 2018 and 2019-2026 reconstruction provenance; zero market-data requests")
        r18=radar.redis.get_json(_boos18u_key("report"),None)
        if not r18 or r18.get("status")!="COMPLETED": raise RuntimeError("Completed v1.7.53 2018 reconstruction report is required")
        if r18.get("result_sha256")!=BACKWARD_OOS_2018_OBSERVED_CERT_SPEC["required_2018_reconstruction_result_sha256"]: raise RuntimeError("2018 reconstruction result SHA mismatch")
        if r18.get("universe_spec_sha256")!=BACKWARD_OOS_2018_OBSERVED_CERT_SPEC["required_2018_reconstruction_spec_sha256"]: raise RuntimeError("2018 reconstruction spec SHA mismatch")
        rh=radar.redis.get_json(radar.universe_reconstruction_key("report"),None)
        if not rh or rh.get("status")!="COMPLETED": raise RuntimeError("Completed frozen 2019-2026 reconstruction report is required")
        if rh.get("reconstruction_sha256")!=BACKWARD_OOS_2018_OBSERVED_CERT_SPEC["required_2019_2026_reconstruction_sha256"]: raise RuntimeError("2019-2026 reconstruction SHA mismatch")

        _boos18oc_set_state(status="RUNNING",phase="RECOUNT",message="Recomputing 2018 observed counts from persisted v1.7.53 records only")
        records=radar.redis.get_json(_boos18u_key("records"),None)
        if not isinstance(records,dict): raise RuntimeError("Persisted v1.7.53 2018 records are missing")
        source_union=len(records)
        observed=sum(1 for rec in records.values() if isinstance(rec,dict) and rec.get("observed_in_2018"))
        exp=BACKWARD_OOS_2018_OBSERVED_CERT_SPEC["2018_expected"]
        if source_union!=exp["source_union_symbols"] or observed!=exp["symbols_observed_in_sip"]:
            raise RuntimeError(f"2018 record recount mismatch: union={source_union}, observed={observed}")

        frozen=BACKWARD_OOS_2018_OBSERVED_CERT_SPEC["2019_2026_frozen_reference"]
        years={str(k):int(v) for k,v in (rh.get("year_presence_counts") or {}).items()}
        if int(rh.get("symbols_with_sip_presence_2019_2026") or -1)!=frozen["symbols_with_sip_presence"]: raise RuntimeError("2019-2026 observed symbol count mismatch")
        if int((rh.get("source_counts") or {}).get("union_symbols") or -1)!=frozen["union_symbols"]: raise RuntimeError("2019-2026 source-union count mismatch")
        if years!=frozen["year_presence_counts"]: raise RuntimeError("2019-2026 annual presence counts mismatch")

        mean=sum(years.values())/len(years)
        diff_2019_pct=(observed/years["2019"]-1.0)*100.0
        diff_mean_pct=(observed/mean-1.0)*100.0
        diagnostics={
            "symbols_observed_2018":observed,"symbols_observed_2019":years["2019"],
            "2019_2026_annual_mean":mean,"2018_vs_2019_pct":diff_2019_pct,
            "2018_vs_2019_2026_mean_pct":diff_mean_pct,
            "coverage_proof":False,"pass_fail_gate":False,
            "interpretation":"Context only; neither comparison estimates true-market coverage."
        }
        report={
            "version":VERSION,"build":BUILD,"certification_id":BACKWARD_OOS_2018_OBSERVED_CERT_SPEC["certification_id"],
            "cert_spec_sha256":BACKWARD_OOS_2018_OBSERVED_CERT_SPEC_SHA256,"status":"COMPLETED","decision":"OBSERVED_UNIVERSE_CERTIFIED",
            "required_2018_reconstruction_result_sha256":r18.get("result_sha256"),"required_2019_2026_reconstruction_sha256":rh.get("reconstruction_sha256"),
            "terminology":BACKWARD_OOS_2018_OBSERVED_CERT_SPEC["terminology"],
            "recomputed_2018":{"source_union_symbols":source_union,"symbols_observed_in_sip":observed},
            "frozen_2019_2026":{"source_union_symbols":frozen["union_symbols"],"symbols_with_sip_presence":frozen["symbols_with_sip_presence"],"year_presence_counts":years},
            "diagnostics":diagnostics,
            "split_warmup_finding":BACKWARD_OOS_2018_OBSERVED_CERT_SPEC["split_warmup_finding"],
            "certification_scope":{
                "observed_universe_methodology_comparable":True,"complete_us_market_coverage_certified":False,
                "all_2018_corporate_actions_certified":False,"h1_validated":False,
                "note":"Original split-suspect logic must be reused unchanged at candidate stage; Phase 0B strict exclusion remains required."
            },
            "scope_guard":{"alpaca_requests_made":0,"h1_computed":False,"mfe_mae_computed":False,"target_adverse_computed":False,"ranking_computed":False,
                "candidate_selection_computed":False,"backward_oos_opened":False,"fresh_forward_oos_opened":False,"2019_2026_results_mutated":False,
                "h1_execution_allowed":False,
                "next_step":"FREEZE/REVIEW a 2018 candidate-census and strict corporate-action exclusion protocol that mirrors the original 2019-2026 methodology before opening H1."},
            "stop_and_review_required":True,"completed_at":iso(),
        }
        report["result_sha256"]=hashlib.sha256(json.dumps(report,sort_keys=True,separators=(",",":"),allow_nan=False).encode()).hexdigest()
        radar.redis.set_json(_boos18oc_key("report"),report)
        _boos18oc_set_state(status="COMPLETED",phase="STOP_REVIEW",message="2018 observed historical SIP universe methodology certified; H1 remains locked",
            decision=report["decision"],result_sha256=report["result_sha256"],alpaca_requests_made=0,h1_execution_allowed=False,stop_and_review_required=True)
    except Exception as exc:
        logging.exception("2018 observed-universe certification failed")
        _boos18oc_set_state(status="ERROR",phase="BLOCKED",message="2018 observed-universe certification failed closed",last_error=f"{type(exc).__name__}: {exc}",alpaca_requests_made=0,h1_execution_allowed=False)
    finally:
        with BACKWARD_OOS_2018_OBSERVED_CERT_LOCK: BACKWARD_OOS_2018_OBSERVED_CERT_THREAD=None

def _boos18oc_start() -> tuple[bool,str]:
    global BACKWARD_OOS_2018_OBSERVED_CERT_THREAD
    if not radar.redis.configured:return False,"Redis is required"
    with BACKWARD_OOS_2018_OBSERVED_CERT_LOCK:
        if BACKWARD_OOS_2018_OBSERVED_CERT_THREAD and BACKWARD_OOS_2018_OBSERVED_CERT_THREAD.is_alive():return False,"already_running"
        BACKWARD_OOS_2018_OBSERVED_CERT_THREAD=threading.Thread(target=_boos18oc_worker,name="ipr-2018-observed-universe-cert",daemon=True)
        BACKWARD_OOS_2018_OBSERVED_CERT_THREAD.start()
    return True,"started"

@app.get("/research/2018-backward-oos/observed-universe/protocol")
def backward_oos_2018_observed_universe_protocol():
    return jsonify({"version":VERSION,"build":BUILD,"cert_spec":BACKWARD_OOS_2018_OBSERVED_CERT_SPEC,"cert_spec_sha256":BACKWARD_OOS_2018_OBSERVED_CERT_SPEC_SHA256,
        "execution_started":False,"alpaca_requests_made":0,"h1_computed":False,"backward_oos_opened":False,"fresh_forward_oos_opened":False,
        "note":"Protocol is outcome-blind and performs zero market-data requests."})

@app.route("/research/2018-backward-oos/observed-universe/start",methods=["GET","POST"])
def backward_oos_2018_observed_universe_start():
    ok,why=_boos18oc_start()
    return jsonify({"ok":ok,"message":why,"status_url":"/research/2018-backward-oos/observed-universe/status","result_url":"/research/2018-backward-oos/observed-universe/result","h1_execution_allowed":False}),(202 if ok else 409)

@app.get("/research/2018-backward-oos/observed-universe/status")
def backward_oos_2018_observed_universe_status():
    persisted=radar.redis.get_json(_boos18oc_key("status"),None) if radar.redis.configured else None
    with BACKWARD_OOS_2018_OBSERVED_CERT_LOCK:
        out=dict(persisted or BACKWARD_OOS_2018_OBSERVED_CERT_STATE);out["worker_alive"]=bool(BACKWARD_OOS_2018_OBSERVED_CERT_THREAD and BACKWARD_OOS_2018_OBSERVED_CERT_THREAD.is_alive())
    return jsonify(out)

@app.get("/research/2018-backward-oos/observed-universe/result")
def backward_oos_2018_observed_universe_result():
