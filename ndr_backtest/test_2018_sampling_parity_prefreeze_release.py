...
class TestSamplingParityPrefreezeRelease(unittest.TestCase):
    def test_version(self): self.assertIn('VERSION = "1.7.55-R1"',TEXT)
    ...
    def test_controls_are_inherited(self): self.assertIn('FEATURE_DISCOVERY_EXEC_SPEC["controls"]["hard_negative_matching"]',TEXT); self.assertIn('FEATURE_DISCOVERY_EXEC_SPEC["controls"]["random_control"]',TEXT); self.assertIn('exact execution parity confirmed from the original feature-discovery loop',TEXT)
