...
class TestSamplingParityPrefreezeRelease(unittest.TestCase):
    def test_version(self): self.assertIn('VERSION = "1.7.55-R2"',TEXT)
    ...
    def test_historical_control_text_not_rewritten(self):
        self.assertIn('same session is a stricter subset of the frozen broad time/regime requirement',TEXT)
        self.assertIn('random_control_parity_finding',TEXT)
        self.assertIn('historical_frozen_artifact_rewritten":False',TEXT)
