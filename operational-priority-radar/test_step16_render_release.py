...
class TestStep16(unittest.TestCase):
 def test_render_is_background_worker(self):
  y=(ROOT/"render.yaml").read_text();self.assertIn("type: worker",y)
 def test_start_command(self):
  y=(ROOT/"render.yaml").read_text();self.assertIn("startCommand: python shadow_worker_main.py",y)
 def test_shadow_forced_in_blueprint(self):
  y=(ROOT/"render.yaml").read_text();self.assertIn('value: "true"',y)
 def test_no_secrets_embedded(self):
  y=(ROOT/"render.yaml").read_text();self.assertIn("sync: false",y);self.assertNotIn("https://",y)
