...
def breakout_follow_through_confirmed(
    ...
    try:
        ...
        print(
            ...
            f"cp>{vwap}={cp > vwap} | "
            f"cp>{ema9}={cp > ema9} | "
            f"min3>{vwap*0.985:.2f}={last_3_closes.min() > vwap * 0.985} | "
            f"min3>{ema9*0.99:.2f}={last_3_closes.min() > ema9 * 0.99}",
            flush=True
        )
        holding_above_vwap_ema = (
            cp > vwap
            and cp > ema9
            and last_3_closes.min() > vwap * 0.98
