...
def load_active_trades_from_redis():
    ...
    if isinstance(saved, dict):
        active_trades = saved
        print(f"✅ Restored Bot 2 active trades from Redis: {len(active_trades)}", flush=True)
    else:
        active_trades = {}
        print("⚠️ No valid Bot 2 Redis active trades found", flush=True)
...
def analyze_symbol(symbol, source_group, df=None):
    try:
        ...
        if not quality_ok:
            print(
                f"❌ Bot 2 quality rejected: {symbol} | {quality_reason}",
                flush=True
            )
            return None
